import React from "react";
import Navbar from "../shared/Navbar";
import { Label } from "../ui/label";
import { Input } from "../ui/input";
import { RadioGroup } from "../ui/radio-group";
import { Button } from "../ui/button";
import { Link } from "react-router-dom";

const Signup = () => {
  return (
    <div>
      <Navbar />
      <div className="flex items-center justify-center mx-auto max-w-7xl">
        <form
          action=""
          className="w-1/2 border border-gray-200 rounded-md my-2 p-4"
        >
          <h1 className="font-bold text-xl mb-5">Sign Up</h1>
          <div className="my-4">
            <Label className='my-1'>Full Name</Label>
            <Input type="text" value="" placeholder="patidar" />
          </div>
          <div className="my-4">
            <Label className='my-1'>Email</Label>
            <Input type="email" value="" placeholder="patidar@gmail.com" />
          </div>
          <div className="my-4">
            <Label className='my-1'>Phone Number</Label>
            <Input type="text" value="" placeholder="12346757657" />
          </div>
          <div className="my-4">
            <Label className='my-1'>Password</Label>
            <Input type="password" value="" placeholder="patidar" />
          </div>

          <div className="flex items-center justify-between">
            <RadioGroup className="flex items-center gap-6 my-5" >
              <div className="flex items-center space-x-2">
                <Input type='radio' name="role" value="student" className='cursor-pointer' id="r1" />
                <Label htmlFor="r1" className='cursor-pointer'>Student</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Input type={'radio'} name="role" value="recruiter" className='cursor-pointer' id="r2" />
                <Label htmlFor="r2" className='cursor-pointer'>Recruiter</Label>
              </div>
            </RadioGroup>

            <div className="flex items-center gap-2">
                <Label>Profile</Label>
                <Input accept="image/*" type="file" className={'cursor-pointer'} />
            </div>
          </div>
          <Button type="submit" className="w-full my-4">Sign Up</Button>
          <span className="text-sm">Already have an account? <Link to="/login" className="text-blue-600" >Login</Link></span> 
        </form>
      </div>
    </div>
  );
};

export default Signup;
